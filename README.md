...
4. **黑名单**：如果配置了 CLUSTER_BLACKLIST，则排除黑名单内的集群
5. **白名单**：如果配置了 CLUSTER_WHITELIST，则仅处理白名单内的集群（黑名单优先级更高）
...
- [DescribeDBClusterAttribute](https://api.aliyun.com/document/polardb/2017-08-01/DescribeDBClusterAttribute) - 查看集群详细属性（返回 StorageType、StorageSpace、StorageUsed、CompressStorageMode 等字段）
